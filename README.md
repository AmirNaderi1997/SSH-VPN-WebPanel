...
2. **عبور از فیلترینگ DPI (DPI Bypass)**:
   ...
   - قابلیت اتصال کاربران از طریق کلاینت‌هایی مانند NapsternetV، HTTP Injector و غیره از طریق CDN (با استفاده از ساب‌دامین متصل به شبکه توزیع محتوا).
...
- `enforce_limits.sh`: اسکریپت بررسی محدودیت حجم و زمان انقضای کاربران (غیرفعال‌سازی خودکار در صورت اتمام دیتا یا زمان).
...
### ۴. تغییر مشخصات ورود به پنل
پیش از شروع کار وب‌پنل، نام کاربری و رمزعبور دلخواه خود را در فایل بک‌اند ویرایش کنید:
فایل `webapp/app.py` را باز کرده و مقادیر متغیرهای زیر را شخصی‌سازی کنید:
```python
ADMIN_USER = "username_delkhah"
ADMIN_PASS = "password_secure_delkhah"
```

### ۵. راه‌اندازی سرویس‌های سیستم‌دی (Systemd Services)
...
### ۶. تنظیم زمان‌بندی فایروال و کرون‌جاب (Cron Jobs)
...
# باز کردن کرون‌جاب سیستم و ثبت دستورات محاسباتی ترافیک
...
## راه‌اندازی SSL و HTTPS با Nginx
...
سلب مسئولیت: مطمئن شوید رکورد A دامنه شما به آی‌پی سرورتان اشاره می‌کند. مقدار `your-domain.com` را با دامنه واقعی خود جایگزین کنید:
```bash
certbot certonly --standalone -d your-domain.com --non-interactive --agree-tos -m your-email@gmail.com
```

### ۳. اعمال پیکربندی Nginx
فایل پیکربندی Nginx (`/opt/vpn_manager/nginx_panel.conf`) را باز کنید و آدرس دامنه‌تان را در بخش `server_name` و مسیرهای مربوط به گواهینامه SSL جایگزین کنید. سپس تنظیمات را فعال کنید:
```bash
...
اکنون می‌توانید از طریق آدرس امن **`https://your-domain.com:8080`** به پنل کاربری دسترسی داشته باشید.
